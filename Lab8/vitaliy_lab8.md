...
```
Медицинский персонал:     500 чел.
Административный персонал: 50 чел.
───────────────────────────────
Итого DAU:                ~550 пользователей
```

### 2. Действия пользователей в день

| Роль | Действия | Кол-во | Итого действий |
|------|---------|--------|---------------|
| Врач | Просмотр карты пациента | 20 | |
| | Обновление назначений | 10 | |
| | Выполнение аналитических запросов | 5 | **~35** |
| Медсестра | Ввод показателей | 30 | |
| | Обновление статуса пациента | 10 | **~40** |
| Админ | Формирование отчётов | 15 | |
| | Управление расписанием | 20 | **~35** |

**Среднее: ~35 действий на пользователя в день**

### 3. RPS (Requests Per Second)

```
Общее количество действий в день:
550 пользователей × 35 действий = 19,250 действий/день

Средний RPS (при равномерной нагрузке за 8 часов):
19,250 / (8 × 3600) ≈ 0.67 RPS

Пиковый RPS (×3 от среднего, утренний час пик):
0.67 × 3 ≈ 2.0 RPS
```
...
```
Средний размер одного действия (обновление записи): ~5 КБ

Данные в день:
19,250 действий × 5 КБ ≈ 96 МБ/день

Данные в год:
96 МБ × 365 ≈ 35 ГБ/год (только текстовые данные)

Медицинские изображения (отдельно):
~100 ГБ/год (рентген, УЗИ, сканы)
```
